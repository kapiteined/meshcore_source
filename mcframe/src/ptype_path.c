...
void ptype_path(const onair_packet_t *pkt) {
    if (pkt->payload_len < 4) {
        fprintf(stderr, "  PATH outer: too_short payload_len=%u (need >=4)\n", (unsigned)pkt->payload_len);
        return;
    }
    ...
    fprintf(stderr, "  PATH outer: dst_hash=0x%02X src_hash=0x%02X mac=0x%04X ciphertext_len=%u\n",
           (unsigned)p[0], (unsigned)p[1], (unsigned)mac, ct_len);
    util_print_undecryptable_ciphertext("PATH", ct, ct_len);
}
