...
int main(void) {
  /* Load keys from current working directory and hard-fail on any error. */
  int rc;

  rc = util_pubkeys_load("./pubkeys.txt");
  if (rc != 0) {
    fprintf(stderr, "fatal: failed to load ./pubkeys.txt (rc=%d)\n", rc);
    return 1;
  }

  rc = util_privkeys_load("./privkeys.txt");
  if (rc != 0) {
    fprintf(stderr, "fatal: failed to load ./privkeys.txt (rc=%d)\n", rc);
    return 1;
  }
