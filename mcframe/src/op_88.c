...
void op_88(const uint8_t *frame, size_t len) {
    if (len < 4) { fprintf(stderr, "PUSH_LOG_RX_DATA (0x88): too_short len=%u\n", (unsigned)len); return; }
    ...
    if (rc != 0) { fprintf(stderr, "PUSH_LOG_RX_DATA (0x88): snr=%.2f dB rssi=%d dBm raw_len=%u (onair_parse rc=%d)\n", snr_db, rssi_dbm, (unsigned)raw_len, rc); return; }
    fprintf(stderr, "PUSH_LOG_RX_DATA (0x88): snr=%.2f dB rssi=%d dBm raw_len=%u\n", snr_db, rssi_dbm, (unsigned)raw_len);
    ptype_dispatch(&pkt);
}
