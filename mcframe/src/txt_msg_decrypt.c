                              ...
                              txt_msg_result_t *out)
{
  ...
  for (i = 0; i < util_privkeys_count(); i++) {
    ...
    for (j = 0; j < util_pubkeys_count(); j++) {
      ...
      util_crypto_hmac_sha256(shared, 32, ciphertext, ct_len, hmac);
      mac_calc = (uint16_t)hmac[0] | ((uint16_t)hmac[1] << 8);
      ...
      memcpy(out->from_pubkey, peer->pub, 32);
      out->from_pubkey_valid = 1;
